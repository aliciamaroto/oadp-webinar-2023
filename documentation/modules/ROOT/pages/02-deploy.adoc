...
----
oc create secret generic cloud-credentials-azure -n openshift-adp --from-file cloud=credentials-velero-azure
----
...
spec:
  backupLocations:
    - velero:
        config:
          resourceGroup: webinar-oadp
          storageAccount: webinaroadp
...
- The default VolumeSnapshotClass must have the `deletionPolicy` set to `Retain` and the label `velero.io/csi-volumesnapshot-class` set to `true`. This resould should look as follows:
