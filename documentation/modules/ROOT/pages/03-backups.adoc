...
Then, we need to prepare the environment in order to use the Data Mover functionality.

- The default `volumeSnapshotClass` should be as follows:
...
- The OADP namespace needs to have the annotation `volsync.backube/privileged-mover` set to true
...
Finally, we need to modify the DataProtectionApplication to include the Data Mover feature:

[source,yaml,subs="+macros,+attributes"]

----
ind: DataProtectionApplication
...
Once the DPA is reconciled, the backups created will use the Data Mover feature by default.
